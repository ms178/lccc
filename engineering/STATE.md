...
- **Boolean mux/majority algebra** (`bit_idioms::match_bool_mux_algebra`, default-on, `CCC_NO_BOOL_ALGEBRA=1` kill switch): the CH/MAJ family — `(x&y)^(x&z)`→`x&(y^z)`, `(x&y)^(~x&z)`→`z^(x&(y^z))` (SHA-256 CH, 4→3 ops — GCC keeps the NOT), `(x&y)^(x&z)^(y&z)`→`(x&y)^(z&(x^y))` (MAJ parity), with the Or-spelling matrix (inner-Or/outer-Xor rejected — different function). Producer repurposes under single-use + operand-dominance proofs, same-block v1, and the COHERENCE CONTRACT: rewrites apply immediately with defs/use_counts maintained (deferred application + stale snapshot = the Pattern-A-then-Pattern-C miscompile the sha256 known-vector check caught). 14 unit tests incl. an exhaustive 6³-pattern truth-table oracle. See `engineering/FOLLOWUP-2026-09-18-wo-audit-and-benchmarks.md`.
