...
1. **`abs_i16` is 17.86× behind gcc-O2** and `sat_add_i16` 5.49× — integer
   `Neg` select lanes. gcc emits `pabsw`; lccc has no packed-abs intrinsic.
   Largest single remaining runtime gap in this family.
2. **`VecMaxU32x8` / `VecMinU32x8`** (SSE4.1 `pmaxud`/`pminud`, already
...
3. `same_source` is still `Copy`-blind (`slp_vectorizer.rs:1106`) — C3 fixed
   `build_pack`, not `same_source`, so a copy-wrapped arm can still miss a
   min/max fold. Missed optimization only.
4. SLP probe remainder, worst first: `abs_i16x8` 91, `sha256_schedule` 38,
   `clamp_u8x16` 28, `copy_bytes` 16, `add4` 14, `scale4`/`narrow_i32_to_i16` 11.
5. Corpus rank (`rank-before.json`, 51 files): total gap 2743; worst
