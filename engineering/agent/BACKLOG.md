...
| 26 | RA-21 | DONE | Vecreg admission now records exact vector-argument prefixes, admits the audited post-whitelist SSE2 binary/insert/zero operations, and keeps generic FP/EVEX families closed until their destination protocol is register-assignment-aware; canonical block/global use counting removes false one-block live-out spills; unary PMOVZX no longer reaches the binary-emitter panic | `regalloc.rs`, `generation.rs`, x86 intrinsics | **M/C/G** sat chain 24 vs 25 instructions and 1 vs 6 vector stack accesses; paired VM 0.9410× control (95% CI 0.9182..0.9627, 31/32); four-oracle gap retained | runtime + structural kill-switch + PMOVZX regression | classify scalar/immediate args as vector pointers or open generic SIMD wholesale |
...
| 72 | OP-13 | P1 | DCE non-escaping/overwritten stores plus loop-invariant intrinsic-chain motion | `dce.rs` all Store live; LICM legality | **G/C** RA-21 sat oracle: LCCC 58 instructions vs Clang 11 (hoists invariant chain and collapses repeated output stores) | dead stores gone; exact checksum and alias guard | DCE volatile or hoist aliasable loads |
