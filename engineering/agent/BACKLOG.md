...
| RA-28b | P0 | **Mode 6 is a prerequisite for RA-06, not a standalone win.** The measurement explains itself: under pure lifetime demotion a *more accurate* cost model evicts more readily, and every eviction is permanent, so an unrolled accumulator chain pays all of the victim's later uses in memory. Mode 6 wins exactly where lifetimes are long and pressure is high (`struct_copy` -51 stkref) and loses where a nearly-dead partial sum is re-read after the loop. Re-measure mode 6 **after** splitting lands; expect the sign to flip because the victim will then be reloaded at its next use instead of demoted for its whole lifetime | `live_range.rs`, `split_ranges.rs` | **M** see RA-28 | kernel stkref negative with splitting on | re-measuring before RA-06 |
