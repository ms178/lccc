...
| RA-06 | **P0 — partial: pass built, measured NEGATIVE, off by default** | `split_high_pressure_ranges` implements Belady-MIN spill-then-color as an IR pre-pass (per-point pressure, farthest-next-use eviction, store + fresh-SSA reload, phi-operand rename). Correct: 633 PASS / 0 FAIL and torture-clean with `CCC_PRESSURE_SPLIT=1`. But it REGRESSES every configuration measured (kernel stkref +57 at the most conservative setting, +119 at the most aggressive; not one function improves) because an **intra-block** splitter cannot reach the MAXLIVE <= k guarantee that makes spill-then-color pay: the dominant candidates are loop-header phi values consumed in another block (32 of them rejected at `arith_loop`'s peak) and renaming those needs cross-block SSA repair. Without the guarantee the program pays for the split traffic AND the colorer's demotions. **Next step is the repair, not tuning** — see `engineering/DECISIONS.md` | `split_ranges.rs` | **M** full sweep in DECISIONS.md; arith_loop 165 vs GCC 92 | cross-block SSA repair lands, then kernel stkref negative | tuning the budget until the aggregate looks flat |
