...
| RA-28 | P0 | **Measure `CCC_EVICT_MODE=6` and decide the default.** The position-relative, per-use-frequency cost model (`LiveRange::{use_weights,suffix_cost,cost_boost}`, `spill_cost_at`) landed with 12 unit tests and mode 6 wired into `select_evict_victim`; only the evidence is missing. Run `ra_quality_census.py --json` (mode 3 vs 6) over programs+kernel_corpus, then `run_benchmarks.py --reps 21` paired on the RA-sensitive set | `live_range.rs` `evict_mode` | **C** `priority` is one scalar for a whole range AND position-independent | census `stkref` down, gzip unregressed, Adler DO8 unregressed | flipping the default without the paired measurement (modes 2/4/5 and both dead-victim variants all died here) |
