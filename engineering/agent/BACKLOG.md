...
| OPT-40 | **P0 — highest-value item found this session** | **Scalar load sinking.** lccc has no scalar equivalent of `vec_load_sink`, so a "load a batch, then compare with early exit" loop keeps every loaded value live from the top of the body. `glibc_memcmp_common_alignment` loads eight words up front and pays six callee-saved pushes plus a 120-byte frame; GCC sinks each load to its own compare and keeps two values live, using ZERO callee-saved registers. Sink a load to the block that uses it when the load is non-volatile, its address is invariant along the path, and no aliasing store intervenes | new pass beside `vec_load_sink.rs` | **M** memcmp 197 vs GCC 87; 6 vs 0 pushes | memcmp callee-saved pushes 6 -> 0 | sinking across an aliasing store or a call |
