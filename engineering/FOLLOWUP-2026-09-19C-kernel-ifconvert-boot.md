...
The earlier 23-instruction reduction preserves LCCC's established deterministic
`Clz(0)/Ctz(0) == width` extension. Applying that compatibility behavior inside
`while (mask)`, however, emitted a redundant test, conditional branch, width
fallback, join branch, and allocator preload despite a dominating nonzero fact.

The IR now distinguishes defined-zero `Clz`/`Ctz` from proven-nonzero
`ClzNonZero`/`CtzNonZero`. Direct frontend builtins retain the deterministic
operation, which is required by the regression corpus. Correlated value
propagation specializes it only when dominating CFG facts prove the source
nonzero, including through same-width and widening integer casts that preserve
the zero partition. Truncations are explicitly rejected. The false/zero edge
retains the defined operation. Baseline x86 and i686 use BSR/BSF without zero
repair on the proven domain, while targets whose native instruction already
defines zero share their existing lowering.
...
changing the source or weakening LCCC's deterministic direct-zero behavior.
