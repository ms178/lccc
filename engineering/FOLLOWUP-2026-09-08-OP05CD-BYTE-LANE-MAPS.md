...
Follow-ups 3.1 (memfold-aware resolution), 3.4 (`vpbroadcastb` + promotion
range recovery), 3.5 (`vpminsb`/`vpmaxsb`), 3.6 (16-bit lanes) and the
testing half of 3.7 are **DONE** this session -- see 1.4e through 1.4i.
Three items remain, in value order.

### 3.1 Redundant second induction variable  (HIGH — every map loop)

Every vectorized map loop still carries both a trip counter and a byte
offset:
...
per iteration in ALL map kernels: the byte, word and dword clamps would go
7 -> 6 (GCC parity) and `fold_lower` 10 -> 9.

Deliberately NOT attempted in two sessions now, and the reason should be
respected: the source IV is bounded by `floor(limit / VF)` and the remainder
loop derives its start index from it, so removing it rewires the loop's exit
semantics AND the remainder entry together.  Getting it wrong is a silent
wrong answer for specific trip counts — the hardest failure mode to notice.

Recipe: give the SOURCE IV the step `VF` (not 1) and bound it by
`limit - limit % VF`; address as `base + iv * elem_size` through the SIB
scale, which is legal for elem_size in {1,2,4,8} and covers every element
type the map path admits; the remainder start is then the IV itself with no
shift.  Validate with the existing trip-count sweeps (they already cover
every n from 0 up at four element widths) plus the differential oracle.

### 3.2 Unroll the packed map body 2×  (MEDIUM)

Clang's remaining lead on every byte kernel is 4× unrolling of identical
per-element work.  Two independent 32-byte streams would amortise the loop
overhead (3 of 7-10 instructions) and fill both load ports.  Expect
`fold_lower` ~15 / 64 B = 0.234 insn/B, beating Clang's 0.242.  Gate on
trip-count and code-size heuristics.

### 3.3 A shift rule for the demotion  (LOW)
