...
# Resolve tools against the caller's working directory before entering K. Real
# build callers use absolute paths, but accepting a relative path at validation
# and then hashing it relative to the kernel tree is a surprising half-contract.
LCCC=$(realpath "$LCCC")
LCCC_LD=$(realpath "$LCCC_LD")
...
if [[ ! -f $compiler_stamp && -f .lccc-tools-stamp ]]; then
  echo "kernel_tool_identity: migrating legacy tool stamp; cleaning all Kbuild products"
  # The legacy combined stamp cannot tell whether the compiler changed. Treat
  # its one-time migration as the stronger event; preserving stale objects here
  # would defeat content-based invalidation precisely on the upgrade build.
  make ARCH=x86_64 clean >/dev/null
  compiler_changed=1
elif [[ -f $compiler_stamp && $(cat "$compiler_stamp") != "$compiler_hash" ]]; then
