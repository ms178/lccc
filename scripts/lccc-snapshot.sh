...
# and ledger under /home/user/artifacts, the git bundle (the one artifact that
# cannot be re-derived from the checkout — it carries the full history for
# arena_session_restore.sh's .git recovery — so it MUST live in the persisted
# zone) under /home/user/artifacts as well, and the source tarball (fully
# derivable, ~9 MB) in the snapshot-excluded bulk zone.
#
...
# Derived bulk artifacts (the 8–9 MB source tarball and the repo-sized git
# bundle) are regenerated by every run and fully derivable from the checkout,
# so they land in the snapshot-EXCLUDED zone by default: the persisted
# workspace stays far below the ~128 MB / 10k-file snapshot caps (the S62
# harness-choke lesson), while `LCCC_BULK_ARTIFACTS` still overrides for a
# run that must publish them.
BULK=${LCCC_BULK_ARTIFACTS:-/home/user/target/artifacts}
mkdir -p "$BULK"
...
# ---- 5. Git bundle (the .git-recovery artifact — DURABLE zone) ----------------
# arena_session_restore.sh rebuilds a wiped .git by cloning
# /home/user/artifacts/lccc.bundle; publishing it into the snapshot-excluded
