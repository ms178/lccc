...
if [[ $RECONFIG == 1 ]] || ! grep -q "^CONFIG_SCHED_BORE=y" .config; then
  ...
  ./scripts/config \
    ...
    --disable HARDENED_USERCOPY --disable HARDENED_USERCOPY_FALLBACK \
    --disable INIT_ON_ALLOC_DEFAULT_ON --disable INIT_ON_FREE_DEFAULT_ON \
    --disable KASAN --disable KMSAN --disable UBSAN --disable KCSAN
...
# Use -Os for main kernel to avoid SSE movdqu in memcpy (nf_conntrack_tcp_init_net
# ICE with -mno-sse) and to keep size reasonable with gvn disabled.
# -g0 reduces debug info and compile time/memory for huge trace files (trace.o).
exec make ARCH=x86_64 CC="$LCCC" LD="$LCCC_LD" AS="$LCCC" KCFLAGS="-Os -g0" -j"$JOBS" "${targets[@]}"
