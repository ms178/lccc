...
def run_census(lccc: str, files: list[Path], opt: str, cflags: list[str]) -> dict:
    ...
    with open(tmp_path) as f:
        data = json.load(f)
    os.unlink(tmp_path)
    # Side-confusion guard: the child records the compiler path it really
    # ran.  A missing env= would silently run the default binary on both
    # sides (every pre-fix A/B measured new-vs-new); refuse that loudly.
    used = data.get("compilers", {}).get("lccc")
    if used != lccc:
        print(f"census_ab: SIDE CONFUSION: requested {lccc}, child ran {used}",
              file=sys.stderr)
        raise SystemExit(2)
    return data
...
def main() -> int:
    ...
    # Absolute compiler paths: the child runs with cwd=REPO, so a relative
    # path would resolve against the wrong directory there.
    args.old = str(Path(args.old).resolve())
    args.new = str(Path(args.new).resolve())
