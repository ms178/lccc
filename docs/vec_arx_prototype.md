...
* Double rounds use the standard diagonalization: lane-rotate rows by
  ...
  halves — no transpose in the loop.
* `rotl16` is FUSED with the diagonal lane rotation: a single
  `vpshufd $147/$57` (4-byte lane rotate == rotl16 per dword) produces the
  rotated value already placed in its diagonal slot. Cost: 0 extra ops.
* `rotl8` uses a `vpshufb` LUT (16-byte control vector) — 1 op.
...
* Loop keeps all live state in xmm5–xmm8 + 5 mask constants: zero spills,
  ~47 dynamic insns per double round ×10 + 20 prologue/epilogue.
...
|---------|-------------------|----------------|------|
| add/xor | 4                 | 1              |      |
| rotl16  | 4                 | 0 (fused w/ diagonal vpshufd) | only in diagonalized position |
| rotl8   | 4                 | 1 (vpshufb LUT) | |
| rotl12/7| 8                 | 6 (3-op shift pairs) | VPROLD is AVX512-only |

Vector QR ≈ 11 vector ops vs 20 scalar ops → ~1.8x op-count win before
register-pressure/spill effects (lccc currently spills 16× on this kernel);
the win compounds because the lane form removes the 16-GPR pressure entirely.
