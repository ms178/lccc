...
static int clz64(unsigned long x) { return __builtin_clzl(x); }
static int ctz64(unsigned long x) { return __builtin_ctzl(x); }
static int pc32(unsigned x) { return __builtin_popcount(x); }
static int pc64(unsigned long x) { return __builtin_popcountl(x); }
...
int main(void)
{
	...
	static const unsigned long w64[] = {
		0x0000000000000001ul, 0x0000000100000000ul, 0x00000000fffffffful,
		0xffffffff00000000ul, 0x8000000000000000ul, 0x7ffffffffffffffful,
		0xaaaaaaaaaaaaaaaaul, 0x5555555555555555ul, 0xfffffffffffffffeul,
	...
	for (unsigned i = 0; i < sizeof(w64) / sizeof(w64[0]); i++) {
		unsigned long x = w64[i];
		h = mix(h, (unsigned long)clz64(x));
		h = mix(h, (unsigned long)ctz64(x));
