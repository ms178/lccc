...
/* P4: mixed-width stores tiling exactly (1+1+2+4+4+4 bytes = 16). The
 * union spelling keeps every access defined C: writing one member and
 * reading the byte array afterwards reads the stored object
 * representation, which the character-member rules define (the previous
 * `(unsigned short *)(buf + 2)` pointer punning violated the effective
 * type rules and could read unaligned). */
static int p4(void) {
    union {
        unsigned char b[16];
        unsigned short h[8];
 ...
 * Union spelling with every read touching bytes a prior write defined
 * (the old version read a[1]'s uninitialized high half through an
 * effective-type-violating cast). */
static int a6(void) {
    union {
        unsigned int w[2];
        unsigned short h[4];
    } u;
    u.h[0] = 0x1122u;          /* bytes 0-1 */
    u.h[2] = 0x0000u;          /* bytes 4-5: defined before the w[1] read */
    u.w[0] = 0x33445566u;      /* bytes 0-3: OVERLAPS h[0]; later store wins */
    u.h[3] = 0x7788u;          /* bytes 6-7 */
    u.w[1] = (u.w[1] & 0xffff0000u) | 0x7788u; /* read-modify-write */
    return (int)(u.w[0] & 0xffu) + (int)((u.w[0] >> 24) & 0xffu) + (int)(u.w[1] & 0xffu);
}
...
/* A8: the address escapes through a TERMINATOR (return). Compiling the
 * shape must not ICE, and the promotion must reject: the rewrite would
 * delete the alloca out from under the returned reference (found live:
 * this ICE'd the backend before terminators were audited).
 *
 * The returned pointer is DANGLING, so evaluating it is undefined and
 * cannot serve as a differential oracle — GCC legitimately folds
 * `p != 0` to 0 for a dead object (observed in CI: the GCC build's own
 * self-check failed while lccc's passed, and the outputs diverged). The
 * corpus therefore keeps the function COMPILED but uncalled (`used`
 * keeps it emitted at every level), and the no-ICE / no-promotion
 * contracts are asserted structurally by check_const_array_promote.sh
 * on the assembly. */
__attribute__((noinline, used)) static int *leaky(void) {
    int a[4];
    for (int i = 0; i < 4; i++) a[i] = i;
    return a;
}
