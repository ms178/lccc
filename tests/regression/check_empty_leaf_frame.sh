#!/usr/bin/env bash
# AB-09: call-free leaves with only callee-save pushes need no duplicate local frame.
set -euo pipefail
...
CCC_NO_LEAF_PARAM_GPR=1 "$CCC" -O2 -S "$t/t.c" -o "$t/on.s"
...
grep -q 'pushq' <<<"$on"
! grep -Eq '(subq|addq).*%rsp' <<<"$on"
grep -Eq 'subq.*%rsp' <<<"$off"
grep -q '\.cfi_def_cfa_offset 24' <<<"$on"
