...
#                   reference to the deleted alloca). The dangling pointer
#                   is never evaluated (evaluating it is UB and no oracle
#                   may compare it — GCC folds `leak() != 0` to 0).
...
__attribute__((noinline)) static int sum_u8(const unsigned char *p, int n) {
    int s = 0;
...
__attribute__((noinline)) static int sum(const int *p, int n) { int s = 0; for (int i = 0; i < n; i++) s += p[i]; return s; }
int probe(void) {
    int a[4];
...
cat > "$tmp/hole.c" <<'EOF'
__attribute__((noinline)) static int rd(const int *p) { return p[0] + p[1]; }
int probe(int k, int v) {
    int a[4];
    for (int i = 0; i < 4; i++) a[i] = i;
    a[k & 3] = v;
...
/* The returned pointer is dangling: evaluating it is undefined and no
 * oracle may compare it (GCC folds `leak() != 0` to 0 for the dead
 * object). Call-and-discard keeps the body compiled and executed without
 * observing the indeterminate value. */
...
__attribute__((noinline)) __attribute__((noinline)) static int rd(const int *p) { return p[0] + p[1]; }
int probe(void) {
    int a[2];
    a[0] = 11;
    a[1] = 22;
...
# Scoped to the promoted object's own emission block: the directive that
# precedes the .LCA_ label (any unrelated .align elsewhere in the TU must
# not satisfy the contract).
lca_align4=$(awk '/^\.align/{last=$0} /^\.LCA_[0-9]+:/{print last; exit}' "$tmp/align4.s")
check_eq "align4: natural alignment preserved" "$lca_align4" ".align 4"
...
lca_align64=$(awk '/^\.align/{last=$0} /^\.LCA_[0-9]+:/{print last; exit}' "$tmp/align64.s")
check_eq "align64: explicit alignment honored" "$lca_align64" ".align 64"
...
cat > "$tmp/write.c" <<'EOF'
__attribute__((noinline)) static int bump(int *p, int n) { for (int i = 0; i < n; i++) p[i] = p[i] + 1; return p[0]; }
int probe(void) {
    int a[4];
    for (int i = 0; i < 4; i++) a[i] = i;
    bump(a, 4);
