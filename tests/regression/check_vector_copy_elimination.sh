...
if [[ "$ch" -eq 4 ]]; then
    ...
    bad "chain should be three instructions plus ret, got $((ch - 1)):"; fnbody "$work/shapes.s" chain | note
fi
...
EOF
    ...
    static void same(const char *name, double got, double want) {
        if (got != want) { mismatches++; printf("MISMATCH %s %.17g != %.17g\n", name, got, want); }
    }
    int main(void) {
        static const double xs[] = {
        ...
        for (int i = 0; i < n; i++) {
            ...
            /* oracle-compared */
            printf("%d %.17g %.17g %.17g %.17g %.17g %.17g %.9g %.9g %.17g %.17g\n", i,
                   b_madd(a, b, c), b_msub(a, b, c), b_nmadd(a, b, c), b_nmsub(a, b, c),
                   b_madd_p(a, b, &c), b_madd_pm(a, &b, c), (double)b_maddf(fa, fb, fc),
                   (double)b_maddf_pm(fa, &fb, fc), b_chain(a, b, c, d), b_reuse(a, b, c));
...
EOF_ALGEBRA
    ...
    if [[ $lccc_rc -ne 0 ]]; then
        ...
        if [[ $rc_l -ne 0 ]]; then
            ...
            ok "FMA algebra kernel bit-exact vs $ORACLE on $(grep -c . "$work/fa.lccc") lines; 15 contracted kernels equal their builtin twins"
        fi
        ...
        if [[ "$n132" -gt 0 && "$n213" -gt 0 && "$n231" -gt 0 && "$nmem_non231" -gt 0 && "$nfam" -ge 3 ]]; then
        ...
        # count.  The builtin sign variants (`fma(-a, b, -c)`) are not in this
        # list: the negations are separate `vxorpd`s today and their fusion
        # into vfnmadd/vfnmsub is the open item in FOLLOWUP-2026-09-19E.
        one_insn_ok=1
        for fn in b_madd b_madd_p b_madd_pm b_maddf b_maddf_pm \
                  ...
                  sq_add self_add self_add2 maddf_a maddf_m nmaddf_c; do
        ...
        if [[ "$one_insn_ok" -eq 1 ]]; then
            ok "all 20 straight-line FMA kernels are exactly one instruction plus ret (gcc parity)"
        fi
    fi
