...
fn try_fold_one_branch_chain(
    ...
    // The dead side is rewritten below; a dead side that IS the block being
    // rewritten (a self-referential chain) needs surgery this fold does not
    // model. Pathological, and not worth the audit surface.
    ...
    // Full-inside (&&): Bexit loses BOTH chain edges — its arms are simply
    // dropped (no arm-equality needed), and when the chain was its only
    // predecessor it is deleted outright (audited).
    // Full-outside (||): Bbody loses its only chain edge — its Bcheck arms
    // are dropped (no retarget: Bcond never flows to Bbody), possibly with
    // the block itself; Bexit keeps its Bcond arm and drops Bcheck.
    let mut drop_bexit = false;
    let mut drop_bbody = false;
    if full {
        if and_form {
            ...
            let Some(&bbody_idx) = idx_of.get(&bbody_label) else {
                return None;
            };
            let check_only = preds
                .row(bbody_idx)
                .iter()
                .all(|&p| func.blocks[p as usize].label.0 == bcheck_label);
            if check_only {
                if !dead_block_is_droppable(func, idx_of, preds, bbody_label, &[bcheck_label]) {
                    return None;
                }
                drop_bbody = true;
            }
    ...
    let (new_insts, new_term) = if full {
        // Constant condition: control flows one way, unconditionally.
        let target = if and_form { bbody_label } else { bexit_label };
        (
            Vec::new(),
            Terminator::Branch(crate::ir::reexports::BlockId(target)),
        )
    } else {
    ...
    // Bbody phis: full-outside drops the Bcheck arms (the edge died; Bcond
    // does not flow to Bbody). Every other case retargets Bcheck arms to
    // Bcond — the bcond→bbody edge exists in all of them.
    if full && !and_form {
        if let Some(&bbody_idx) = idx_of.get(&bbody_label) {
            let bbody = &mut func.blocks[bbody_idx];
            for inst in &mut bbody.instructions {
                let Instruction::Phi { incoming, .. } = inst else {
                    break;
                };
                incoming.retain(|(_, f)| f.0 != bcheck_label);
    ...
    } else if let Some(&bbody_idx) = idx_of.get(&bbody_label) {
        let bbody = &mut func.blocks[bbody_idx];
    ...
    // Bexit phis: full-inside loses BOTH chain arms; every other case
    // keeps the Bcond arm and drops Bcheck (proven value-equal above in
    // the non-full form).
    if full && and_form {
    ...
    // Dead-block drops, by label, after every index-based surgery.
    if drop_bexit {
        drop_dead_block(func, bexit_label);
    }
    if drop_bbody {
        drop_dead_block(func, bbody_label);
    }
    func.blocks.retain(|b| b.label.0 != bcheck_label);
    Some(())
}
...
mod tests {
    ...
    /// INT_MAX` guarding the exit — constant FALSE, so Bcond branches to
    /// the exit unconditionally and the never-reached body is deleted.
    #[test]
    fn full_domain_branch_chain_outside_folds_to_unconditional() {
        ...
        assert_eq!(f.blocks.len(), 2, "Bcheck and Bbody removed");
        assert!(matches!(
            &f.blocks[0].terminator,
            Terminator::Branch(BlockId(3))
        ));
