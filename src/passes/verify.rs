        ...
        for (ii, inst) in block.instructions.iter().enumerate() {
            if let Instruction::Phi { incoming, .. } = inst {
                ...
                for (op, from) in incoming {
                    ...
                    let Some(&pred) = label_to_idx.get(from) else { continue };
            ...
            } = inst
            {
                ...
                let mut check_op = |out: &mut Vec<Violation>, op: &Operand, what: &str| {
                    ...
                    check_value_use(
                        func, stage, &defs, reachable, &dom, v.0, bi, Some(ii), &site, out,
                    );
                };
                ...
                inst.for_each_used_value(|v| {
                    ...
                    check_value_use(
                        func, stage, &defs, reachable, &dom, v, bi, Some(ii), &site, out,
                    );
                });
            }
        }
...
fn check_value_use(
    ...
    let Some(&(db, di)) = defs.get(&v) else {
        out.push(detail(format!("use of undefined value v{} in {}", v, use_site)));
        return;
    };
    if !reachable[db] {
    ...
    let ok = dom.dominates(db, use_block)
        && (db != use_block || use_idx.is_none_or(|ui| di < ui));
    if !ok {
        out.push(detail(format!(
