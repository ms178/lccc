...
fn extract_window<'a>(expr: &'a MapExpr, bits: u32) -> Option<(&'a MapExpr, u64, u64)> {
    ...
    if let MapExpr::MaskConj { is_and: true, l, r } = expr {
        let (xl, bl) = classify_unsigned_bound(l, bits)?;
        let (xr, br) = classify_unsigned_bound(r, bits)?;
        if xl != xr {
            return None;
        }
        return match (bl, br) {
            (RangeBound::Lower(lo), RangeBound::Upper(hi)) => Some((xl, lo, hi)),
            (RangeBound::Upper(hi), RangeBound::Lower(lo)) => Some((xl, lo, hi)),
            _ => None,
        };
    }
    let (x, bound) = classify_unsigned_bound(expr, bits)?;
    Some(match bound {
