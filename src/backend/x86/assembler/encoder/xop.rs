...
impl InstructionEncoder {
    fn emit_xop_prefix(
        ...
        self.bytes
            .push((r_bit << 7) | (x_bit << 6) | (b_bit << 5) | map);
        self.bytes
            .push((v_prime_bit << 7) | (vvvv_inv << 3) | (l << 2));
    }
}
...
impl InstructionEncoder {
    ...
    fn encode_lwp(&mut self, ops: &[Operand], reg_field: u8, mnemonic: &str) -> Result<(), String> {
        if ops.len() != 3 {
        ...
        let (vvvv, v_prime) = match &ops[2] {
            Operand::Register(r) => {
                let id = gp_id(&r.name).ok_or_else(|| format!("bad LWP register: {}", r.name))?;
    ...
    fn encode_xop_4op(
        ...
        if !ymm_capable && widths.iter().any(|w| *w == Some(1) || *w == Some(2)) {
    ...
    fn encode_xop_unary(
        ...
        if ops
            .iter()
            .any(|op| matches!(op, Operand::Register(r) if is_ymm(&r.name)))
        {
            return Err(format!("operand size mismatch for `{mnemonic}'"));
        }
        let (src_name, dst_name) = match (&ops[0], &ops[1]) {
            ...
            _ => {
                return Err(format!(
                    "unsupported XOP horizontal operands for `{mnemonic}'"
                ));
            }
        };
        let src_num =
            xop_vec_id(src_name).ok_or_else(|| format!("bad XOP register: {src_name}"))?;
        let dst_num =
            xop_vec_id(dst_name).ok_or_else(|| format!("bad XOP register: {dst_name}"))?;
        let r = needs_vex_ext(dst_name);
