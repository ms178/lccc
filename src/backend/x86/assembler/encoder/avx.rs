...
impl super::InstructionEncoder {
    ...
    pub(crate) fn encode_vpermil2(
        ...
        let sel = match &ops[0] {
            Operand::Immediate(ImmediateValue::Integer(v)) => {
                if !(0..=15).contains(v) {
                    return Err(format!(
                        "constant doesn't fit in 4 bits for `{mnemonic}'"
                    ));
                }
                *v as u8
            }
        ...
        match (&ops[1], &ops[2]) {
            (Operand::Register(src1), Operand::Register(src2)) => {
                ...
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, dst_num, reg_num(&src2.name).ok_or("bad register")?));
    ...
    pub(crate) fn encode_evex_expandcompress(
        ...
        match (&ops[0], &ops[1]) {
            ...
            (Operand::Register(src), Operand::Memory(mem)) if is_compress => {
                let src_num = self.emit_evex_memop(
                    &src.name, mem, None, 2, w, 1, ll, z, aaa, false,
                )?;
                self.bytes.push(opcode);
            ...
            (Operand::Memory(mem), Operand::Register(dst)) if !is_compress => {
                let dst_num = self.emit_evex_memop(
                    &dst.name, mem, None, 2, w, 1, ll, z, aaa, false,
                )?;
                self.bytes.push(opcode);
                let (_, scale_n) = Self::evex_mem_scale(mem, ll, tuple_div);
                self.encode_evex_mem(dst_num, mem, scale_n)
        ...
        match (&ops[0], &ops[1]) {
            (Operand::Register(src1), Operand::Register(src2)) => {
                let (dst_num, rm_num) = self.emit_evex_mod3(
                    &base, &src1.name, Some(&src2.name), 2, w, pp, ll, z, aaa, false,
                )?;
                self.bytes.push(opcode);
            ...
            (Operand::Memory(mem), Operand::Register(src2)) => {
                ...
                let (bcst, scale_n) = Self::evex_mem_scale(mem, ll, 1);
                let dst_num = self.emit_evex_memop(
                    &base, mem, Some(&src2.name), 2, w, pp, ll, z, aaa, bcst,
                )?;
                self.bytes.push(opcode);
                self.encode_evex_mem(dst_num, mem, scale_n)
            }
    ...
    pub(crate) fn encode_evex_narrow(
        ...
        Self::apply_evex_sae(sae, EvexSae::None, 0)?;
        let src = match &ops[0] {
            Operand::Register(r) if is_xmm(&r.name) || is_ymm(&r.name) || r.name.to_lowercase().starts_with("zmm") => r,
            _ => return Err("operand type mismatch".to_string()),
        };
        ...
        match &ops[1] {
            Operand::Register(dst) => {
                let (src_num, dst_num) =
                    self.emit_evex_mod3(&src.name, &dst.name, None, 2, 0, 2, src_ll, z, aaa, false)?;
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, src_num, dst_num));
                Ok(())
            }
            Operand::Memory(mem) => {
                let src_num = self.emit_evex_memop(
                    &src.name, mem, None, 2, 0, 2, src_ll, z, aaa, false,
                )?;
                self.bytes.push(opcode);
    ...
    pub(crate) fn encode_evex_vcvtps2ph(&mut self, ops: &[Operand]) -> Result<(), String> {
        ...
        match &ops[2] {
            Operand::Register(dst) => {
                let (src_num, dst_num) =
                    self.emit_evex_mod3(&src.name, &dst.name, None, 3, 0, 1, src_ll, z, aaa, false)?;
            ...
            Operand::Memory(mem) => {
                let src_num = self.emit_evex_memop(
                    &src.name, mem, None, 3, 0, 1, src_ll, z, aaa, false,
                )?;
                self.bytes.push(0x1D);
    ...
    pub(crate) fn encode_evex_shuffle32x4(
        ...
        let xmm = ops.iter().any(
            |op| matches!(op, Operand::Register(r) if is_xmm(&r.name)),
        );
        if xmm {
            return Err(format!("operand size mismatch for `{mnemonic}'"));
        }
        ...
        match ops.len() {
            ...
            2 => match (&ops[0], &ops[1]) {
                (Operand::Memory(mem), Operand::Register(dst)) => {
                    let (aaa, z) = Self::evex_mask_info(&ops[1]);
                    let dst_num = self.emit_evex_memop(
                        &dst.name, mem, None, 1, w, pp, 0, z, aaa, false,
                    )?;
                    self.bytes.push(0x10);
                ...
                (Operand::Register(src), Operand::Memory(mem)) => {
                    let (aaa, z) = Self::evex_mask_info(&ops[1]);
                    let src_num = self.emit_evex_memop(
                        &src.name, mem, None, 1, w, pp, 0, z, aaa, false,
                    )?;
