...
pub(crate) fn check_broadcast_shape(
    ...
    let dst_ok = match mnemonic {
        ...
        "vbroadcastf32x4" | "vbroadcastf64x2" | "vbroadcasti32x4" | "vbroadcasti64x2" => {
            dst_w >= 1
        }
        "vbroadcastf32x8" | "vbroadcastf64x4" | "vbroadcasti32x8" | "vbroadcasti64x4" => {
            dst_w == 2
        }
        _ => evex || dst_w <= 1,
    };
...
impl super::InstructionEncoder {
    ...
    fn evex_sae_class(map: u8, opcode: u8) -> EvexSae {
        match (map, opcode) {
            ...
            // NOTE: 0x5B (vcvtps2dq/vcvtdq2ps/...) is deliberately None:
            // no convert supports SAE/ER (`unknown vector operation').
    ...
    pub(crate) fn encode_evex_vcvt(
        ...
        let ll = if params.kind == VcvtKind::Narrow {
            match &ops[0] {
        ...
        match (&ops[0], &ops[1]) {
            (Operand::Register(src), Operand::Register(dst)) => {
                let (dst_num, src_num) = self.emit_evex_mod3(
                    &dst.name, &src.name, None, params.map, params.w, params.pp, ll, z, aaa, false,
                )?;
