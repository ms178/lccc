...
fn check_movdq_shape(mnemonic: &str, ops: &[Operand]) -> Result<(), String> {
    ...
    let has_mm = ops.iter().any(|op| {
        matches!(op, Operand::Register(r) if matches!(reg_class(&r.name), RegClass::Mmx))
    });
    let has_xmm = ops.iter().any(|op| {
        matches!(op, Operand::Register(r) if matches!(reg_class(&r.name), RegClass::Xmm))
    });
    if has_mm && has_xmm {
        return Err(type_err());
    }
