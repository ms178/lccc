...
//! with.  (One deliberate exception sits outside this module's contract: when
//! a NaN is PRODUCED, the payload — and its sign — may depend on operand
//! position, which is implementation-defined in IEEE-754; two forms may
//! therefore return bitwise-different NaNs while agreeing on NaN-ness and on
//! every real-valued result.  The regression gate compares with a
//! payload-insensitive predicate for exactly this reason.)  What a change of
//! form DOES change is two things the surrounding code cares about:
//!
...
impl<'a> FmaRoles<'a> {
    ...
    ///
    /// When `dst` is the addend the form is 231 — unless `dst` names a
    /// multiplicand as well (`x * x + a` with the result in x), where the
    /// multiplicand forms are equally legal and `prefer` decides.  When it is
    /// only a multiplicand the other multiplicand and the addend share
    /// operands 0 and 1, and the memory operand — if there is one — decides
    /// which goes where.  With two registers either form is legal and
    /// `prefer` breaks the tie, so that an instruction re-encoded without a
    /// change of destination keeps its text; failing a usable preference, 213
    /// is chosen, which is the form Clang and ICX emit for
    /// `__builtin_fma(a, b, c)`.
    pub(super) fn encode(&self, dst: u8, prefer: FmaForm) -> Option<ScalarFma<'a>> {
        ...
        let (form, op0, op1) = if xmm_reg(self.addend) == Some(dst) {
            ...
            // The destination may name a multiplicand as well (`x * x + a`
            // with the result in x): 231 is one legal encoding, and so is
            // whichever multiplicand form `prefer` names.  Honouring the
            // preference keeps the promise that an unchanged destination
            // reproduces the original text even in that aliased corner.
            let dst_in_mult = xmm_reg(m2) == Some(dst) || xmm_reg(m3) == Some(dst);
            if dst_in_mult && matches!(prefer, FmaForm::F132 | FmaForm::F213) {
                let other = if xmm_reg(m2) == Some(dst) { m3 } else { m2 };
                // The addend is the destination register here, so it can
                // never be the memory operand; `other` can, and 213 reads
                // its operand 1, so a memory multiplicand forces 132.
                if prefer == FmaForm::F213 && !is_memory(other) {
                    (FmaForm::F213, self.addend, other)
                } else {
                    (FmaForm::F132, other, self.addend)
                }
            } else if is_memory(m2) {
                (FmaForm::F231, m2, m3)
            } else {
                (FmaForm::F231, m3, m2)
            }
...
mod tests {
    ...
    /// role register becomes the destination, placement of a memory operand,
    /// AND aliasing pattern among the register operands, the re-encoding must
    /// (1) exist exactly when the ISA has one, (2) compute the same roles,
    /// (3) write the requested register, (4) keep any memory operand at
    /// operand 0, (5) parse back to itself, and (6) reproduce the original
    /// text when the destination is unchanged.
    ///
    /// Aliasing is not decoration: `mentions()` counts multiplicity, and
    /// `x * x + a` with the result in x takes the tie branch of `encode()`
    /// that distinct registers never reach.  The patterns cover every way
    /// the three operand slots can share a register; with a memory operand
    /// at operand 0 only the register slots can alias.
    #[test]
    fn encode_preserves_roles_for_every_form_destination_and_memory_placement() {
        // (r0, r1, r2) register numbers for the operand slots, with a memory
        // operand replacing r0 in the `mem` variants.  `distinct` is the
        // historical pattern; the rest alias.
        const DISTINCT: [u8; 3] = [1, 2, 3];
        const OP0_EQ_OP1: [u8; 3] = [1, 1, 3];
        const OP1_EQ_DST: [u8; 3] = [1, 2, 2];
        const OP0_EQ_DST: [u8; 3] = [1, 2, 1];
        const ALL_EQ: [u8; 3] = [1, 1, 1];
        let mut checked = 0usize;
        let mut aliased_checked = 0usize;
        for f in FAMILIES {
            for form in FORMS {
                for w in WIDTHS {
                    for (pattern, aliased) in [
                        (DISTINCT, false),
                        (OP0_EQ_OP1, true),
                        (OP1_EQ_DST, true),
                        (OP0_EQ_DST, true),
                        ...
                        let [r0, r1, r2] = pattern;
                        // Memory in the original line's operand 0 (the only
                        // slot with an r/m encoding), or none.
                        for op0 in ["16(%rdi)", "%xmm1"] {
                            let op0 = if op0 == "%xmm1" {
                                format!("%xmm{r0}")
                            } else {
                                op0.to_string()
                            ...
                            for dst in 0u8..16 {
                                ...
                                let back =
                                    parse_scalar_fma(&rendered).expect("re-encoded line parses");
                                assert_eq!(back.render(), rendered);
                                // Re-encoding without changing the destination
                                // reproduces the original text exactly -- in
                                // every pattern, including the aliased corner
                                // where the destination is addend and
                                // multiplicand at once.
                                if dst == fma.dst() {
                                    assert_eq!(enc.render(), line, "aliased pattern {pattern:?}");
                                }
                            }
                        }
                    }
        ...
        // 4 families × 3 forms × 2 widths × 18 re-encodings each.  Per
        // family/form/width the dst count is: distinct 3+2 (register vs
        // memory operand 0), op0==op1 2+2, op1==dst 2+1, op0==dst 2+2,
        // all-equal 1+1.  216 total, 156 of them over aliased operands.
        assert_eq!(checked, 4 * 3 * 2 * (3 + 2 + 2 + 2 + 2 + 1 + 2 + 2 + 1 + 1));
        assert_eq!(aliased_checked, 4 * 3 * 2 * (2 + 2 + 2 + 1 + 2 + 2 + 1 + 1));
    }
