...
//! stripped.
//!
//! It is not hypothetical.  A pass that derives an operand by cutting
//! `dest_token.len()` bytes off a line slice silently shifts the boundary when
//! the line carries trailing blanks.  The address-copy kill test in
//! `local_patterns.rs` had exactly that bug (a genuine full-width kill was
//! reported as a use, so the fold was lost), and the class was fixed at one site
//! while `len() - ` arithmetic remained at dozens of others.  This module is the
//! check that finds the rest, and the reason a new one cannot land silently.
//!
//! Where the property is enforced: at the two places text enters `LineStore`.
//! `LineStore::new` shortens every span past its trailing blanks and
//! `LineStore::replace` truncates them from the replacement, so no pass can
//! obtain padded text through `store.get(..)` — and `LineInfo::trimmed`, which
//! 473 call sites feed exactly that text, is a pure slice with nothing left to
//! trim.  Enforcing it once at the writers rather than at every reader is also
//! what makes it free: trimming per access inside the fixed-point loop cost 24%
//! of a 33 s kernel-TU compile with `str::trim_end` and still 6% with an ASCII
//! byte scan, for work the store had already done.
//!
