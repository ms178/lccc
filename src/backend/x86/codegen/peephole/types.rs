...
impl LineInfo {
    ...
    /// cached offset, trailing blanks already gone because the store removed
    /// them.
    ///
    /// The trailing-blank story is load-bearing, not cosmetic.  Passes derive
    /// operands by slicing this text (`&t[dest_token.len()..]`,
    /// `split_once(", ")`, `ends_with(reg_name)`), and a line that carries
    /// trailing blanks shifts every one of those boundaries: with four blanks
    /// after `movq (%rcx), %rax` the address-copy fold stopped firing, and a
    /// label spelled `f: ` stopped classifying as a label because the classifier
    /// reads the LAST byte to find the colon.  Trailing whitespace is
    /// semantically neutral in every assembly syntax GAS accepts, so no pass may
    /// observe it.
    ///
    /// It is removed once, where text enters the store: `LineStore::new`
    /// shortens every span past its trailing blanks and `LineStore::replace`
    /// truncates them from the replacement (both calling
    /// `peephole_common::line_len_without_trailing_blanks`), and those two are
    /// the store's only writers — its whole API is `new`, `get`, `len`,
    /// `replace`.  Every `&str` a pass can hold therefore came through one of
    /// them, and all 473 call sites of this accessor pass `store.get(..)`,
    /// directly or through a local bound to it.
    ///
    /// Trimming a second time per access is unreachable work in the peephole's
    /// innermost loop, and it measured: on `mm/page_alloc.c` (7764 lines, kernel
    /// flags, order-rotated interleaved medians) `str::trim_end` here cost 7.8 s
    /// of a 33 s compile (+24%), because it decodes UTF-8 backwards through a
    /// `char::is_whitespace` predicate; the ASCII byte-scan replacement still
    /// cost 1.9 s (+6%).  With the trim at the store only, the compile is back
    /// to baseline (33.3 s) and the emitted object is byte-identical, down to
    /// the boot image's sha256.
    ///
    /// Two things keep that argument honest rather than hopeful.
    /// `peephole_common`'s `the_store_never_hands_a_pass_trailing_whitespace`
    /// pins the store side under all seven paddings, CRLF included, across
    /// `new`, `get`, `build_result` and `replace`; and the `debug_assert` below
    /// names the store as the only legal source, so a future caller handing this
    /// accessor text from anywhere else fails instead of silently reintroducing
    /// the sensitivity `passes/whitespace_invariance.rs` exists to forbid.
    /// (`fastbuild` inherits `release`, so `debug_assertions` are off in the
    /// shipped profile and the test, not the assert, is the load-bearing guard.)
    #[inline]
    pub(super) fn trimmed<'a>(&self, line: &'a str) -> &'a str {
        debug_assert!(
            crate::backend::peephole_common::line_len_without_trailing_blanks(line.as_bytes())
                == line.len(),
            "trimmed() got text with trailing blanks {line:?}: line text must come from \
             LineStore, which strips them at new()/replace()"
        );
...
pub(super) fn classify_line(raw: &str) -> LineInfo {
    ...
    // the classification.  `LineStore` strips trailing blanks from the text it
    // hands out and `LineInfo::trimmed` slices that text unchanged, so this uses
    // the same helper: a pass sees exactly the text that was classified, even
    // when what is being classified did not come from the store.
    let lead = &raw[trim_start..];
    let s =
        &lead[..crate::backend::peephole_common::line_len_without_trailing_blanks(lead.as_bytes())];
    let sb = s.as_bytes();
