        ...
        match val {
                ...
                if let Some(slot) = self.state.get_slot(v.0) {
                    ...
                    if self.state.is_alloca(v.0) {
                        if let Some(align) = self.state.alloca_over_align(v.0) {
                            ...
                            self.state
                                .out
                                .emit_instr_rbp_reg("    leaq", slot.0, &tmp_reg);
                        ...
                        self.state
                            .out
                            .emit_instr_rbp_reg("    movq", slot.0, &tmp_reg);
                    }
            ...
            Operand::Const(c) => {
                ...
                if let Some(addr) = c.to_i64() {
                    ...
                    self.state
                        .out
                        .emit_instr_imm_reg("    movabsq", addr, &tmp_reg);
                    op.mem_addr = format!("(%{})", tmp_reg);
                    return true;
                }
            }
        ...
        if matches!(op.kind, AsmOperandKind::X87St0 | AsmOperandKind::X87St1) {
            match val {
                ...
                Operand::Const(c) => {
                    ...
                    self.state
                        .emit_fmt(format_args!("    {} (%rsp)", fld_instr));
        ...
        match val {
            Operand::Const(c) => {
                ...
                if imm == 0 {
                    ...
                    self.state
                        .out
                        .emit_instr_reg_reg("    xorl", &reg32, &reg32);
                } else if imm >= i32::MIN as i64 && imm <= i32::MAX as i64 {
                    self.state
                        .out
                        .emit_instr_imm_reg("    movq", imm, effective_reg);
                    ...
                    self.state
                        .out
                        .emit_instr_imm_reg("    movabsq", imm, effective_reg);
                }
            }
            Operand::Value(v) => {
                if let Some(slot) = self.state.get_slot(v.0) {
                    if self.state.is_alloca(v.0) {
                        ...
                        if let Some(align) = self.state.alloca_over_align(v.0) {
                            self.state
                                .out
                                .emit_instr_rbp_reg("    leaq", slot.0, lea_reg);
                            self.state.out.emit_instr_imm_reg(
                                "    addq",
                                (align - 1) as i64,
                                lea_reg,
                            ...
                            self.state
                                .out
                                .emit_instr_imm_reg("    andq", -(align as i64), lea_reg);
                        } else {
                            self.state
                                .out
                                .emit_instr_rbp_reg("    leaq", slot.0, lea_reg);
        ...
        if matches!(op.kind, AsmOperandKind::X87St0 | AsmOperandKind::X87St1) {
            ...
            if let Some(slot) = self.state.get_slot(ptr.0) {
                ...
                if self.state.is_direct_slot(ptr.0) {
                    ...
                    self.state
                        .out
                        .emit_instr_rbp_reg("    movq", slot.0, scratch);
                    self.state
                        .emit_fmt(format_args!("    {} (%{})", fld_instr, scratch));
            ...
            if self.state.is_direct_slot(ptr.0) {
                ...
                if is_xmm {
                    ...
                    let dest_reg = Self::asm_load_dest_reg(load_instr, reg);
                    {
                        let sr = self.slot_ref(slot.0);
                        self.state
                            .emit_fmt(format_args!("    {} {}, {}", load_instr, sr, dest_reg));
                    }
                }
            } else {
                ...
                if is_xmm {
                    ...
                    self.state
                        ...
                        .emit_instr_rbp_reg("    movq", slot.0, scratch);
                    self.state
                        .emit_fmt(format_args!("    {} (%{}), %{}", load_instr, scratch, reg));
                    self.state.out.emit_instr_reg("    popq", scratch);
                } else {
                    ...
                    let dest_reg = Self::asm_load_dest_reg(load_instr, reg);
                    self.state
                        .emit_fmt(format_args!("    {} (%{}), {}", load_instr, reg, dest_reg));
                }
            }
        }
    }
    ...
    fn store_output_from_reg(
            ...
            if let Some(slot) = self.state.get_slot(ptr.0) {
                ...
                if self.state.is_direct_slot(ptr.0) {
                    ...
                    self.state
                        ...
                        .emit_instr_rbp_reg("    movq", slot.0, &scratch);
                    self.state
                        .emit_fmt(format_args!("    {} (%{})", fstp_instr, scratch));
                    if preserve {
                        self.state.out.emit_instr_reg("    popq", &scratch);
                    }
                }
        ...
        if let AsmOperandKind::ConditionCode(ref cond) = op.kind {
            ...
            if self.state.get_slot(ptr.0).is_none() {
                ...
                self.state
                    .out
                    .emit_instr_reg_reg("    movzbl", &reg8, &Self::reg_to_32(reg));
            ...
            if let Some(slot) = self.state.get_slot(ptr.0) {
                ...
                if self.state.is_alloca(ptr.0) {
                    ...
                    };
                    {
                        let sr = self.slot_ref(slot.0);
                        self.state
                            .emit_fmt(format_args!("    {} {}, {}", store_instr, src_reg, sr));
                    }
                    ...
                    // full 8-byte store to match value_to_reg's movq load.
                    {
                        let sr = self.slot_ref(slot.0);
                        self.state
                            .emit_fmt(format_args!("    movq %{}, {}", reg, sr));
                    }
                } else {
            ...
            if is_xmm {
                ...
                if self.state.is_direct_slot(ptr.0) {
                    ...
                    self.state
                        ...
                        .emit_instr_rbp_reg("    movq", slot.0, &scratch);
                    self.state
                        .emit_fmt(format_args!("    {} %{}, (%{})", store_instr, reg, scratch));
                    if preserve {
                        self.state.out.emit_instr_reg("    popq", &scratch);
                    }
                }
                ...
                if self.state.is_alloca(ptr.0) {
                    ...
                    let reg64 = Self::reg_to_64(reg);
                    {
                        let sr = self.slot_ref(slot.0);
                        self.state
                            .emit_fmt(format_args!("    movq %{}, {}", reg64, sr));
                    }
                }
            } else {
                // Non-alloca: slot holds a pointer, store through it.
