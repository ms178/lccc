...
//!
//! # Algorithm: iterative equivalence classes
//!
...
//! target — where a local target's identity is *its section's class*.
//! Classes start from a structural hash (local targets as holes) and are
//! refined to a fixpoint: a class whose members disagree splits, which can
//! only cascade into further splits, so the loop provably terminates after
//! at most `n` passes over `n` members (each non-final pass strictly grows
//! the class count). This is the gold/lld design, and it is what makes the
//! `ta`/`tb` case above come out right: the tables land in different
//! classes because their *bytes* differ, so the getters can never be equal.
//!
...
/// Refinement passes are capped. Each pass that splits re-sorts every member
/// position, so an N-link reference chain would otherwise cost N passes of
/// `O(N log N)`; past the cap only classes whose comparison evidence changed
/// under the last pass shatter (see `shatter_unstable`) — everything else
/// keeps its exact classes, so legitimate deep code loses no earned folds.
/// 32 is generous: real links settle in a handful of passes (an N-chain
/// peels one link per pass and needs the cap past 32 links, at which point
/// its exact result is singletons anyway — precisely what the shatter
/// produces).
const MAX_REFINEMENT_PASSES: u32 = 32;

/// Global comparison budget: every exact member comparison and every class
/// sketch charges one unit against 32 per member. Legitimate links spend a
/// small multiple of N (a member is compared a handful of times before its
/// class settles); past the budget the unstable remainder shatters instead
/// of refining further. Together with the pass cap this bounds refinement
/// at `O(N log N)` time and `O(N)` comparisons on every input.
const COMPARE_BUDGET_PER_MEMBER: u64 = 32;
...
/// concrete local section each target denotes. Globals hash by name (same
/// name is the same entity link-wide); absolute locals by value (same value
/// is the same entity); section locals contribute only their in-section
/// offset, leaving the section itself as the hole the fixpoint resolves.
/// Corrupt indices (no symbol) hash raw — buckets are candidates, and exact
/// comparison splits them conservatively. Reserved-index locals hash by
/// symbol index, mirroring the same-symbol rule in
/// [`local_targets_equal`]: hashing only the value would bucket distinct
/// extended sections together.
fn skeleton_hash(obj: &Elf64Object, si: usize) -> u64 {
    let mut h = FxHasher::default();
...
fn targets_equal(
    ...
    match (
        ...
        (Some(a), Some(b)) => {
            ...
            if ga {
                return a.name.as_str() == b.name.as_str();
            }
...
fn members_equal(
    ...
    for (x, y) in ra.iter().zip(rb.iter()) {
        ...
        if !targets_equal(objects, pos_of, class_of, a.id.0, x, b.id.0, y) {
            return false;
        }
    }
    true
...
/// control-flow operand (`call rel32`/`jmp rel32`/`jcc rel32`), or does the
/// instruction compute an address the program can observe? The byte before
/// the displacement decides — `E8`/`E9`/`0F 8x` are the only x86-64 forms
/// whose relocatable field is preceded by those bytes:
/// RIP-relative data access lands on a ModRM (`0x05..0x3D`, never `E8/E9`
/// or `8x`), so `lea`/`mov`/`cmp` and data bytes all correctly report
/// "address". Out-of-range offsets (corrupt input) report "address": the
/// conservative direction, since this feeds the taken-set, never folding.
#[inline]
        ...
        for (si, relas) in obj.relocations.iter().enumerate() {
            ...
            for r in relas {
                match r.rela_type {
                    ...
                    R_X86_64_PC64 => {
                        if !src_exec || !is_call_or_jump(src_data, r.offset, 8) {
                            mark(obj, oi, r.sym_idx);
                        }
                    }
                    // Short branches never carry relocations from real
                    // assemblers (in-range targets resolve internally, the
                    // rest relax to 32-bit); a `PC8/PC16` reloc in the wild
                    // is hand-made, so treat its target as observed.
...
fn class_sketch(objects: &[Elf64Object], pos_of: &[Vec<u32>], class_of: &[u32], m: &Member) -> u64 {
...
/// Budget exhausted while classifying `order[bounds[bi - 1]..]` (the current
/// group onwards). In pass 1 the untouched groups were never compared at
/// all, so they shatter eagerly (each member its own class — sound, since
/// shattering only withholds folds); in later passes the entering classes
/// are proven-equal, so the caller's `shatter_unstable` decides them with
/// the change-set. Either way classification stops here. The current and
/// remaining groups leave `settled`: a surviving record must imply a
...
fn exhaust(
    ...
    ctx: &mut RefineCtx<'_>,
    next_class: &mut u32,
    entering_proven: bool,
) -> RefineOutcome {
    let mut j = bi - 1;
    while j + 1 < bounds.len() {
        ctx.settled.remove(&class_of[order[bounds[j]] as usize]);
        j += 1;
    }
    if !entering_proven {
        let mut j = bi - 1;
...
fn refine_once(
    ...
    // Positions sort by a packed u64: `build_universe` pushes members in
    // (object, section) order, so position order IS id order and
    // `(class << 32) | pos` compares exactly like `(class, id)` — with a
    // single-word comparator and 4-byte moves. The caller owns `order` and
    // reuses the final grouping for collection (classes no longer change,
    // so the last pass's order stays valid).
        ...
        while qi < queue.len() {
            ...
            if same.len() * 2 >= piece.len() {
                ...
                for &m in &piece {
                    ...
                    sk.push((
                        class_sketch(objects, pos_of, class_of, &members[m as usize]),
                        m,
                    ));
                }
                sk.sort_unstable();
...
/// another pass could split nothing there. Global names, absolutes,
/// reserved indices and out-of-universe sections never change class, so
/// only universe-local targets are examined. Relocation-free groups additionally
...
fn shatter_unstable(
    ...
    while start < order.len() {
        ...
        'scan: for &m in group {
            let id = members[m as usize].id;
            let obj = &objects[id.0];
            if let Some(relas) = obj.relocations.get(id.1) {
                for r in relas {
                    let Some(s) = obj.symbols.get(r.sym_idx as usize) else {
                        continue;
                    };
                    ...
                    if s.shndx == SHN_ABS || s.shndx >= SHN_LORESERVE {
                        continue;
                    }
                    if let Some(p) = universe_pos(pos_of, id.0, s.shndx) {
                        if changed.contains(&class_of[p as usize]) {
                            unstable = true;
                            break 'scan;
                        }
                    }
...
pub fn plan(objects: &[Elf64Object], safe_only: bool, dead: &FxHashSet<SecId>) -> IcfPlan {
    ...
    let (members, pos_of) = build_universe(objects, dead);
    if members.len() < 2 {
        return out;
    }
    ...
    // Refine to the fixpoint, within the pass cap and the comparison
    // budget. `order` is owned here and reused by collection afterwards;
    // `shatter_unstable` re-sorts it when it runs, so collection always sees
    // a valid grouping.
...
mod tests {
    ...
    fn address_taken_blocks_folding_in_safe_mode() {
        ...
        assert!(
            plan(&[o.clone()], true, &no_dead()).is_empty(),
    ...
    fn lea_address_taken_blocks_safe_fold() {
        ...
        let p = plan(&[o.clone()], true, &no_dead());
        assert!(
            p.is_empty(),
    ...
    fn excluded_sections_are_not_members() {
        ...
        for safe in [true, false] {
            let p = plan(&[o.clone()], safe, &no_dead());
            assert!(
                p.is_empty(),
    ...
    fn chain_refinement_is_bounded_and_exact() {
        ...
        assert_eq!(p.result.iterations, MAX_REFINEMENT_PASSES);
        assert!(
            p.result.comparisons <= COMPARE_BUDGET_PER_MEMBER * NLINK as u64,
            "budget overrun: {}",
            p.result.comparisons
        );
        assert!(p.result.shattered > 0, "cap stop shattered nothing");
    }
