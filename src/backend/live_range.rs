...
pub struct LiveRange {
    ...
    /// When `spans_loop`: how many of the range's use points lie inside
    /// the spanned loop body. A span used only once or twice per pass is
    /// a loop-carried RECURRENCE slot (arith_loop's accumulators): its
    /// spill load sits directly on the carried dependency chain, and no
...
pub(crate) fn mark_loop_spanning(
    ...
    for range in ranges.iter_mut() {
        ...
        for (((header_start, latch_end), peak), b) in
            loop_extents.iter().zip(&peaks).zip(&bars)
        {
        ...
        let mut exposed_uses = exposed_in_extents.get(&range.value_id).copied().unwrap_or(0);
        if let Some(members) = members_of.get(&range.value_id) {
            for &m in members {
                in_loop_uses = in_loop_uses
                    .saturating_add(uses_in_extents.get(&m).copied().unwrap_or(0));
                exposed_uses = exposed_uses
                    .saturating_add(exposed_in_extents.get(&m).copied().unwrap_or(0));
            }
        }
        if !in_loop_use {
...
impl LinearScanAllocator {
    ...
    pub fn allocate_range(&mut self, range: LiveRange) {
        ...
        if range.spans_loop && self.ra_config.loop_span_reserve > 0 {
            ...
            let reserve = (range.span_reserve as usize)
                .min(self.ra_config.loop_span_reserve);
            ...
            // out of the pool.
            if range.span_marked
                && !range.span_has_in_loop_use
                && self.total_spans > allowed
            {
                self.allocate_spill_slot(range.value_id);
                return;
            }
    ...
    /// `MAX_VALVE_SPAN_FUTURE_USES`. Ties break to the farthest next use
    /// (Braun–Hack MIN: the victim whose register is least urgently
    /// needed). Falls through to the ordinary eviction when no span
    /// qualifies — today's behavior for every loop whose spans carry
    /// recurrences or still have reads to serve (sha256's schedule,
    /// fannkuch's counters, matmul's base pointers).
    fn find_span_valve_victim(&self, incoming: &LiveRange) -> Option<usize> {
        ...
        for (idx, interval) in self.active.iter().enumerate() {
            ...
            if fut > MAX_VALVE_SPAN_FUTURE_USES {
                continue;
            }
            let nxt = next_use_after(&interval.range, incoming.start);
            if fut < best_future || (fut == best_future && nxt > best_next_use) {
...
mod tests {
    ...
    fn mark_loop_spanning_measures_the_block_local_peak() {
        ...
        mark_loop_spanning(&mut ranges, &[(100, 500)], &FxHashMap::default(), &empty_fn());
    ...
    fn mark_loop_spanning_covers_only_whole_extents() {
        ...
        mark_loop_spanning(&mut ranges, &[(50, 390)], &FxHashMap::default(), &empty_fn());
        let flag = |rs: &[LiveRange], id: u32| {
            rs.iter().find(|r| r.value_id == id).unwrap().spans_loop
        };
    ...
    fn loop_span_never_evicts_a_short_range() {
        ...
        alloc.run();
        assert!(alloc.assignments.contains_key(&1), "short range keeps its home");
        assert!(!alloc.assignments.contains_key(&2), "span must not evict it");
    ...
    fn valve_gives_exhausted_short_a_nonrecurrent_span() {
        ...
        alloc.run();
        assert!(!alloc.assignments.contains_key(&1), "span demoted by the valve");
        assert!(alloc.assignments.contains_key(&2), "short takes the register");
    ...
    fn valve_protects_recurrence_carried_spans() {
        ...
        alloc.run();
        assert!(alloc.assignments.contains_key(&1), "carried span keeps its home");
        assert!(!alloc.assignments.contains_key(&2));
    }
    ...
    fn valve_ignores_spans_with_too_many_future_uses() {
        ...
        alloc.run();
        assert!(alloc.assignments.contains_key(&1), "read-rich span keeps its home");
        assert!(!alloc.assignments.contains_key(&2));
    }
    ...
    fn valve_prefers_the_fewest_future_uses_then_farthest_next_use() {
        ...
        alloc.run();
        assert!(!alloc.assignments.contains_key(&2), "1-future-use span demoted");
        assert!(alloc.assignments.contains_key(&1), "2-future-use span keeps its home");
        assert!(alloc.assignments.contains_key(&3), "short takes the freed register");
    }
    ...
    fn mark_loop_spanning_detects_web_recurrence() {
        ...
        let mut f = empty_fn();
        use crate::ir::reexports::{Instruction, Operand, Value, BlockId, IrBinOp};
        use crate::common::types::IrType;
        ...
        let member_of: FxHashMap<u32, u32> = [(2u32, 1u32), (4u32, 3u32)]
            .into_iter()
            .collect();
        let mut ranges = vec![
        ...
        assert!(acc.span_recurrence, "acc web self-reference must be detected");
        assert!(!independent.span_recurrence, "body-computed web is not carried");
