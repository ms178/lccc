...
fn base_chain_contains_iv(
    ...
    while let Some(id) = stack.pop() {
        ...
        match inst {
            Instruction::GetElementPtr { base, offset: Operand::Value(off), .. } => {
            ...
            Instruction::BinOp { lhs, rhs, .. } => {
                if let Operand::Value(v) = lhs { operands.push(v.0); }
                if let Operand::Value(v) = rhs { operands.push(v.0); }
            }
            Instruction::Cast { src: Operand::Value(v), .. } => operands.push(v.0),
            Instruction::Copy { src: Operand::Value(v), .. } => operands.push(v.0),
...
fn build_indexed_gep_map(
    ...
    //
    // Kill switch: `CCC_NO_PF06_ADD Peel` (any non-empty value) disables
    // ONLY the add(iv, const) / sub(iv, const) peeling (the IV used as
    // the SIB index is then the add's RESULT, not the IV — preserves
    // correctness when this soundness gate is too narrow).
