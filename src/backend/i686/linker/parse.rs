...
fn elf_range<'a>(data: &'a [u8], offset: u32, size: u32,
                 filename: &str, what: &str) -> Result<&'a [u8], String> {
    let start = offset as usize;
    let end = start.checked_add(size as usize).ok_or_else(||
        format!("{}: {} range overflows", filename, what))?;
    data.get(start..end).ok_or_else(||
        format!("{}: {} extends past end of file", filename, what))
}
...
fn rel_addend(section: &[u8], offset: u32, rel_type: u32,
              filename: &str, section_name: &str) -> Result<i32, String> {
    if rel_type == R_386_NONE { return Ok(0); }
    ...
    let bytes = section.get(off..off + width).ok_or_else(|| format!(
...
pub(super) fn parse_elf32(data: &[u8], filename: &str) -> Result<InputObject, String> {
    ...
    if e_shentsize < 40 {
        return Err(format!("{}: bogus e_shentsize {} (need >= 40)", filename, e_shentsize));
    }
    if e_shnum == 0 {
        return Err(format!("{}: no section headers", filename));
    }
    let table_size = e_shnum.checked_mul(e_shentsize).ok_or_else(||
        format!("{}: section-header table size overflows", filename))?;
    let table_end = e_shoff.checked_add(table_size).ok_or_else(||
        format!("{}: section-header table offset overflows", filename))?;
    if table_end > data.len() {
        return Err(format!("{}: section-header table extends past end of file", filename));
    }
        ...
        if addralign > 1 && !addralign.is_power_of_two() {
            return Err(format!(
                "{}: section header {} has invalid sh_addralign 0x{:x} \
                 (must be 0 or a power of two)", filename, i, addralign));
        }
    ...
    let shstrtab_data = elf_range(data, shstrtab_hdr.offset, shstrtab_hdr.size,
                                  filename, "section-name string table")?;
    ...
    for (i, shdr) in shdrs.iter().enumerate() {
        ...
        if shdr.entsize < 16 {
            return Err(format!("{}: symbol table has invalid entsize {}", filename, shdr.entsize));
        }
        let str_idx = shdr.link as usize;
        let str_shdr = shdrs.get(str_idx).ok_or_else(|| format!(
            "{}: symbol table links invalid section {}", filename, str_idx))?;
        strtab_data = elf_range(data, str_shdr.offset, str_shdr.size,
                                filename, "symbol string table")?;
        symtab_idx = Some(i);
        break;
    }
    ...
    if let Some(si) = symtab_idx {
        ...
        let sym_data = elf_range(data, sym_shdr.offset, sym_shdr.size,
    ...
    for (i, shdr) in shdrs.iter().enumerate() {
        if shdr.sh_type == SHT_REL {
            if shdr.entsize != 0 && shdr.entsize < 8 {
                return Err(format!("{}: relocation section {} has invalid entsize {}",
                                   filename, i, shdr.entsize));
            }
            if shdr.info as usize >= shdrs.len() {
                return Err(format!("{}: relocation section {} targets invalid section {}",
    ...
    for (i, shdr) in shdrs.iter().enumerate() {
        ...
        let sec_data = if shdr.sh_type != SHT_NOBITS && shdr.size > 0 {
            elf_range(data, shdr.offset, shdr.size, filename,
                      &format!("section '{}' data", sec_name))?.to_vec()
        } else {
        ...
        if let Some(rel_indices) = rel_map.get(&i) {
            for &ri in rel_indices {
                ...
                let rel_data = elf_range(data, rel_shdr.offset, rel_shdr.size,
                                         filename, "relocation section")?;
                ...
                for entry in rel_data.chunks_exact(entsize) {
                    ...
                    let addend = rel_addend(&sec_data, r_offset, rel_type,
                                            filename, &sec_name)?;
...
mod tests {
    ...
    fn implicit_rel_addends_use_the_relocation_field_width() {
        ...
        assert_eq!(rel_addend(&bytes, 0, R_386_PC8, "x.o", ".text").unwrap(), -128);
        assert_eq!(rel_addend(&bytes, 1, R_386_PC16, "x.o", ".text").unwrap(), -32716);
        assert_eq!(rel_addend(&bytes, 3, R_386_PC32, "x.o", ".text").unwrap(), 0);
    }
