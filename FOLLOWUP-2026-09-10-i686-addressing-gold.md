...
| 2 | Octal reinterpretation of leading-zero literals is an unjustified compatibility change | **Agree**. Restored the decimal contract (`"010"` = 10). GAS treats `010` as octal 8 — a documented, deliberate divergence (historical lccc contract; kernel sources do not rely on GAS octal). | in-file test `literal_parsing_preserves_decimal_leading_zeros` |
| 3 | Tests did not exercise real relocation emission | **Agree**. New in-file tests build `InstructionEncoder` instances and assert relocation type/offset/addend/diff_symbol/zero-field through `add_relocation`, `encode_modrm_mem`, `encode_abs_addr_*`. | `relocation_offsets_point_at_displacement_fields` et al. |
...
| 5 | "Transactional" guarantee needed qualification | **Agree**. Rewrite validates *everything* (register names, scale, ESP-index, mode, mixed widths, relocation width/diff/modifier) before the first `self.bytes.push`. | `validation_before_mutation_leaves_encoder_clean` |
